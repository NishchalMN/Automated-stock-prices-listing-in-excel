...
import threading
urls = ['https://in.tradingview.com/symbols/NSE-MRPL', 'https://in.tradingview.com/symbols/NSE-GAIL', 'https://in.tradingview.com/symbols/NSE-ONGC']

# for comp in range(12, 16):
    # try:
    # name = sheet.cell(row = comp, column = 2)
if __name__ == "__main__": 
    t1 = threading.Thread(target=company, args=(urls[0],)) 
    
    t2 = threading.Thread(target=company, args=(urls[1],)) 
    t1.start() 
    t2.start() 
    # t3 = threading.Thread(target=company, args=(urls[2],)) 
    # t3.start() 
    t1.join()
    t2.join()
    # t3.join()
